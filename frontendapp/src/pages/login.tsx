...
// import { useFetchUsers } from "../components/userfetch";
...
//                 {users.map(user => (
//                     <div key={user.name} className="userlist">
...
export const Login = () => {
    ...
    const handleLogin = (values: User) => {
        const user = users.find(user => user.name.toLowerCase() === values.name.toLowerCase());
        if (user && user.email === values.email) {
            nav('/mainpage', { state: { id: user.id, name: user.name } }); // Pass ID and Name
        } else {
            alert("Invalid username or email");
        }
    };
